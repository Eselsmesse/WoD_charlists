...
> Статус: REVIEW · Фаза: 2 · Зависит от: SPEC-003, SPEC-004, SPEC-005
...
- [x] CRUD персонажа работает; `owner` проставляется из запроса, не из тела.
- [x] Список — лёгкий (без всех трейтов), detail — полный.
- [x] Bulk-апсейв трейтов: повторный вызов обновляет рейтинги, не плодит строки;
      неизвестный `code` трейта → 400 с понятным сообщением.
- [x] Доступ к чужому персонажу → 404.
- [x] `rules` эндпоинты доступны без авторизации, только чтение.
- [x] `creation-rules` отдаёт 7/5/3, 13/9/5, freebie, поколения из данных.
...
## Итоги реализации (2026-06-12) — статус REVIEW (в стеке над SPEC-004)

Ветка `spec-006-characters-api` построена поверх `spec-004-rules-data`
(эндпоинту `creation-rules` нужны модель `CreationRule` и данные). Вливается
в dev вместе со SPEC-004 после валидации данных владельцем.

- `rules` read-only API (`AllowAny`): `/api/v1/lines/`, `/lines/{code}/`,
  `traits/?category=&subgroup=`, `clans/` (дисциплины кодами), `archetypes/`,
  `creation-rules/` (`{"rules": {key: value}, "generations": [...]}`).
- `characters` CRUD через `CharacterViewSet`: list — лёгкий сериализатор,
  detail — с вложенными traits/tags; связи line/clan/nature/demeanor по `code`
  с валидацией принадлежности линейке; `owner` — из запроса (`perform_create`).
- `PUT /characters/{id}/traits/` — bulk-апсерт по `code` (update_or_create),
  валидация неизвестных кодов (400 с перечислением) и `rating <= max_rating`;
  rating 0 допустим (обнуление).
- `POST /characters/{id}/copy/` — дубликат с трейтами/меритами/флоу,
  имя «… (копия)», `is_public=False`.
- Изоляция владельца: кверисет фильтруется по `owner` (чужой id → 404) +
  объектный `IsOwner`. Фильтры списка: `?line=`, `?tag=`, `?group=`, `?search=`.
- N+1 закрыт `select_related`/`prefetch_related`.
- Тесты: 19 новых (CRUD, 404 на чужого, bulk идемпотентность/валидация, copy,
  фильтры, rules read-only) — всего 50, OK.
